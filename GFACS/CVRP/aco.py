...
class ACO():
    ...
    def gen_path(self, require_prob=False, invtemp=1.0, paths=None):
        ...
        ##################################################
        while not done:
            if paths is not None and i+1==len(paths):
                break
            selected = paths[i + 1] if paths is not None else None
            actions, log_probs = self.pick_move(prob_mat[prev], visit_mask, capacity_mask, require_prob, invtemp, selected, prev)
    ...
    def pick_move(self, dist, visit_mask, capacity_mask, require_prob, invtemp=1.0, selected=None, prev=None):
        ...
        epsilon =1.0#0.30#0.1# 扰动因子
        prev_zero_mask = (prev == 0)#prev != 0#(prev == 0).all()
        random_vals = torch.rand_like(dist.probs[:, 0])
        epsilon_mask = random_vals < epsilon
        sample_mask = epsilon_mask#&prev_zero_mask#epsilon_mask#prev_zero_mask | epsilon_mask#epsilon_mask| prev_zero_mask#prev_zero_mask & epsilon_mask #& epsilon_mask#prev_zero_mask | epsilon_mask
