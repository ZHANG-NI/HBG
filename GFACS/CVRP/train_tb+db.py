...
def back_calculate(costs_nls, paths_nls, n_nodes, device,paths_raw):
    costs_nls=costs_nls.max()-costs_nls+1#cost越大越好
    cost_matrix = torch.zeros(size=(n_nodes, n_nodes), device=device)
    count_matrix =torch.zeros(size=(n_nodes, n_nodes), device=device)
    z_scores=torch.zeros(size=(n_nodes, n_nodes), device=device)
    cdf_values =torch.zeros(size=(n_nodes, n_nodes), device=device)
    for cost, path in zip(costs_nls, paths_nls):
        for i in range(len(path) - 1):
            x, y = path[i], path[i + 1]  
            cost_matrix[x, y] += cost  
            count_matrix[x, y] += 1  
    #count_matrix[count_matrix == 0] = 1
    average_cost_matrix = cost_matrix #/ count_matrix
    mean_val = torch.mean(average_cost_matrix[count_matrix != 0])
    std_val = torch.std(average_cost_matrix)
    z_scores[count_matrix != 0] = (average_cost_matrix[count_matrix != 0] - mean_val) / std_val
    cdf_values[count_matrix != 0] = torch.tensor(stats.norm.cdf(z_scores[count_matrix != 0].cpu().numpy()),dtype=torch.float32, device=device)

    # 约束 CDF 在 [0, 1] 之间，避免超出概率范围
    cdf_values = torch.clamp(cdf_values, min=0.0, max=1.0)
    cdf_values+=EPS
    cdf_values.fill_diagonal_(1)
    ...
    path_edge_probs[valid_mask] = cdf_values[start_nodes[valid_mask], end_nodes[valid_mask]]

    return torch.log(path_edge_probs)
...
def Z_calculate(paths, x, device):
    ...
    col_indices = torch.arange(1, Z.shape[1] + 1, device=Z.device, dtype=torch.float32).T
    Z = Z / col_indices
    col_indices = torch.arange(2, Z.shape[1] + 2, device=Z.device, dtype=torch.float32).T
    Z_b = Z_b / col_indices
...
def train_instance(
    ...
    for pyg_data, demands, distances, positions in data:
        heu_vec, logZ, x_1 = model(pyg_data, return_logZ=True)
        ...
        forward, forward_pro = forward_calculate(paths.T, heu_mat, DEVICE)
        ...
        reward_f = torch.roll(reward, shifts=1, dims=1)
        reward_f[:, 0] = 0
        reward_nls = gen_costs(distances, paths_nls)
        reward_nls = reward_nls-reward_nls.mean(0)
        reward_f_nls = torch.roll(reward_nls, shifts=1, dims=1)
        reward_f_nls[:, 0] = 0
        reward_f = reward_f - reward_f.mean(0)
        reward_f_nls = reward_f_nls - reward_f_nls.mean(0)
        weighted_advantage = cost_w * reward_nls + (1 - cost_w) * reward
        weighted_advantage_f = cost_w * reward_f_nls + (1 - cost_w) * reward_f
        #reward_f = reward_f - reward_f.mean(0)
        ##################################################
        # Loss from paths before local search
        #print(forward.size(), Z.size())
        forward_flow = forward + Z - (weighted_advantage_f.detach())* beta#logZ #Z
        backward_flow = back_pro - (weighted_advantage.detach())* beta +Z_b
        db_loss = torch.pow(forward_flow - backward_flow, 2).mean()
        sum_loss += db_loss
        weighted_advantage = cost_w * advantage_nls + (1 - cost_w) * advantage
        ...
        if guided_exploration:
            ...
            log_b_f_matrix_nls, back_pro_nls = calculate_log_pb_uniform1(paths_nls.T)
            forward_nls, forward_pro_nls = forward_calculate(paths_nls.T, heu_mat, DEVICE)
            Z_nls, Z_b_nls = Z_calculate(paths_nls, x_nls, DEVICE)
            forward_flow_nls = forward_nls + Z_nls - (reward_f_nls[feasible_idx].detach())* beta#logZ #Z
            backward_flow_nls = back_pro_nls - (reward_nls[feasible_idx].detach())* beta +Z_b_nls
            ...
            db_loss_nls = torch.pow(forward_flow_nls - backward_flow_nls, 2).mean()
            sum_loss_nls += db_loss_nls
            forward_flow_nls = log_probs_nls.sum(0) + logZ_nls.expand(n_ants_nls)  # type: ignore
            backward_flow_nls = calculate_log_pb_uniform(paths_nls.T) - advantage_nls.detach() * beta  # type: ignore
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", type=str, default="../pretrained/cvrp_gfacs_tb+db_pb",
